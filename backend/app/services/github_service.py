...
def format_review_as_markdown(review_result: dict) -> str:
    ...
    if issues:
        ...
        for issue in issues:
            ...
            lines.append(f"- {emoji} **{severity.capitalize()}**: {description}")
        lines.append("")

    lines.append("---")
    lines.append("*Generated by PM Agent AI*")
...
async def post_github_review_comment(
    ...
    """Post an AI review as a comment on a GitHub PR. Returns the comment ID or None on failure."""
    try:
...
async def handle_pull_request(db: AsyncSession, payload: dict) -> None:
    ...
    # Auto-trigger code review when a PR is opened
